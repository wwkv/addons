...
export default function SplitModal({ tx, cats, onSave, onClose}) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "var(--card)", borderRadius: 16, padding: 24, maxWidth: 540, width: "90%", border: "1px solid var(--border)", boxShadow: "0 20px 50px rgba(0,0,0,0.5)" }}>
        <h3 style={{ margin: "0 0 4px", fontSize: 18, fontWeight: 400, fontFamily: "var(--font-display)", color: "var(--text)", display: "flex", alignItems: "center", gap: 8 }}><Scissors size={16} strokeWidth={1.8} />Transactie splitsen</h3>
        ...
        {splits.length === 2 && (
          <div style={{ marginBottom: 14, padding: "0 4px" }}>
            <input type="range" min={5} max={95} value={splits[0].percentage} onChange={e => { const v = parseInt(e.target.value); setSplits([{ ...splits[0], percentage: v }, { ...splits[1], percentage: 100 - v }]); }} style={{ width: "100%", accentColor: "var(--accent)" }} />
          </div>
        )}
          ...
          {splits.map((s, i) => {
            ...
            return (
              <div key={i} style={{ display: "flex", gap: 6, alignItems: "center", padding: "6px 8px", background: "var(--bg-80)", borderRadius: 6, borderLeft: `3px solid ${cat ? cat.color : "#555"}` }}>
                ...
                {splits.length > 2 && <button onClick={() => { const n = splits.filter((_, j) => j !== i); n[0].percentage += s.percentage; setSplits(n); }} style={{ background: "none", border: "none", color: "var(--danger)", cursor: "pointer", display: "flex" }}><X size={14} /></button>}
              </div>
            );
          })}
        </div>
        <div style={{ display: "flex", gap: 8, justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={onClose} style={{ padding: "7px 14px", borderRadius: 7, border: "1px solid var(--border)", background: "transparent", color: "var(--text)", cursor: "pointer", fontSize: 12 }}>Annuleer</button>
            <button onClick={() => { const tp = splits.reduce((a, x) => a + x.percentage, 0); onSave(splits.map(s => ({ ...s, percentage: Math.round(s.percentage / tp * 100) }))); }} disabled={splits.some(s => !s.categoryId)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 16px", borderRadius: 7, border: "none", background: splits.some(s => !s.categoryId) ? "var(--border)" : "var(--primary)", color: splits.some(s => !s.categoryId) ? "var(--muted)" : "#fff", cursor: splits.some(s => !s.categoryId) ? "default" : "pointer", fontSize: 12, fontWeight: 600 }}><Check size={13} />Opslaan</button>
          </div>
        </div>
      </div>
    </div>
